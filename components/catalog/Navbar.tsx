    ...
    <div className={styles.shell}>
          ...
          {[0, 1].map((group) => (
            <div className={styles.tickerGroup} key={group} aria-hidden={group === 1}>
              {tickerItems.map((item, i) => (
                <span key={`${group}-${item}`} className={styles.tickerItem}>
                  {i > 0 && <span className={styles.tickerDot} aria-hidden="true" />}
                  {item}
                </span>
              ))}
            </div>
      ...
      <header className={styles.inner}>
        ...
        <nav className={styles.nav} aria-label="Navegacion principal">
          {navItems.map((item) => (
            <Link className={styles.navLink} href={item.href} key={`${item.href}-${item.label}`}>
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className={styles.actions}>
          <Link className={styles.iconButton} href="/catalogo" aria-label="Buscar productos">
            <SearchIcon />
          </Link>
          <a className={styles.whatsapp} href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" aria-label="Contactar por WhatsApp">
            <WAIconSmall />
